...
public abstract class Calendar extends JComponent {
    ...
    protected static final int MIN_HEIGHT = 550;
    ...
    private void drawDayHeadings() {
        ...
        for (int i = getStartDay().getValue(); i <= getEndDay().getValue(); i++) {
            ...
            String text = dayOfWeek.getDisplayName(TextStyle.SHORT, Locale.ENGLISH) + " " + monthIndextoLetter(day.getMonthValue()) + " " + day.getDayOfMonth();
            x = (int) (dayToPixel(DayOfWeek.of(i)) + (dayWidth / 2) - (FONT_LETTER_PIXEL_WIDTH * text.length() / 2));
            g2.drawString(text, x, y);
        }
    }

    private String monthIndextoLetter(int index) {
        switch(index){
            case 1: return "Jan";
            case 2: return "Feb";
            case 3: return "Mar";
